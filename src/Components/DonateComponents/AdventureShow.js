...
class AdventureShow extends React.Component {
  ...
  componentDidMount() {
    fetch(
      `http://localhost:3000/api/v1/adventures/${this.props.match.params.id}`
    )
      .then(res => res.json())
      .then(adventure => {
        this.setState({
    ...
    if (this.state.adventure.sum_amount - this.state.adventure.cost > 0) {
      return (
        <p className="charity-amount">
          ${this.state.adventure.sum_amount - this.state.adventure.cost} Donated
          to Charity
        </p>
      );
    } else {
  ...
  render() {
    return (
      <div className="adventure-show">
        ...
        <h4>Fund Goal: ${this.state.adventure.cost}</h4>
        <h4>
          ${this.state.adventure.sum_amount} Donated of $
          {this.state.adventure.cost}
        </h4>
        {this.amountToCharity()}
        <ProgressBar
          percentage={
            (this.state.adventure.sum_amount / this.state.adventure.cost) * 100
          }
        />
        <br />
        <br />
        ...
        <DonateDialog
          updateAdventureState={this.updateAdventureState}
